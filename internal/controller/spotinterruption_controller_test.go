...
var _ = Describe("SpotInterruption Controller", func() {
	Context("When reconciling a resource", func() {
		...
		It("should successfully reconcile the resource", func() {
			...
			By("Checking if the resource is processed")
			Eventually(func(g Gomega) {
				g.Expect(k8sClient.Get(ctx, ktypes.NamespacedName{Name: spotInterruption.Name}, &spotInterruption)).To(Succeed())
				g.Expect(spotInterruption.Status.ProcessedAt.UTC()).To(Equal(fakePassiveClock.Now()))
			}).Should(Succeed())
		})

		It("should delete an expired SpotInterruption", func() {
			By("Creating an expired SpotInterruption object")
			spotInterruption := spothandlerv1.SpotInterruption{
				ObjectMeta: metav1.ObjectMeta{
					GenerateName: "test-spotinterruption-",
				},
			...
			spotInterruption.Status.ProcessedAt = metav1.Date(2006, 1, 3, 0, 0, 0, 0, time.UTC)
